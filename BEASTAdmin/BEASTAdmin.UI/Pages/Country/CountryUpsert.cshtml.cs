...
public partial class CountryUpsertModel : PageModel
{
    [BindProperty]
    public CountryModel Country { get; set; }

    [BindProperty, DisplayName("Country Image")]
    public IFormFile? CountryImage { get; set; }

    public string ErrorMessage { get; set; }
    public string SuccessMessage { get; set; }
    //public string ImageURL { get; set; }

    private readonly ILogger<CountryUpsertModel> _logger;
    private readonly IConfiguration _config;
    private readonly CountryService _countryService;
	private readonly IWebHostEnvironment _hostEnvironment;

	public CountryUpsertModel(ILogger<CountryUpsertModel> logger, IConfiguration config, CountryService countryService, IWebHostEnvironment hostEnvironment)
	{
        this._logger = logger;
        this._config = config;
    ...
    TryCatch(async () =>
    {
        ...
        {
            Country = await _countryService.GetCountryById(id);
        }
    ...
    TryCatch(async () =>
    {
        ...
        if (string.IsNullOrEmpty(Country.Id))
        {
            ...
            await _countryService.InsertCountry(Country, logModel);
            SuccessMessage = InformationMessages.Saved;
        }
        else
        {
            Country.ModifiedBy = User.Identity.Name;
            await _countryService.UpdateCountry(Country.Id, Country, logModel);
