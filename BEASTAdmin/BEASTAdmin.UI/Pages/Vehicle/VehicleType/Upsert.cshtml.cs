...
public partial class UpsertModel : PageModel
{
    ...
    [BindProperty, DisplayName("VehicleType Image")]
    public IFormFile? VehicleTypeImage { get; set; }
	...
	public UpsertModel(ILogger<UpsertModel> logger, IConfiguration config, VehicleTypeService vehicleTypeService, CategoryService categoryService, IWebHostEnvironment hostEnvironment)
	{
        this._logger = logger;
        this._config = config;
    ...
    TryCatch(async () =>
    {
        ...
        {
            VehicleType = await _vehicleTypeService.GetVehicleTypeById(id);
        }
    ...
    TryCatch(async () =>
    {
        ...
        if (string.IsNullOrEmpty(VehicleType.Id))
        {
            ...
            await _vehicleTypeService.InsertVehicleType(VehicleType, logModel);
            SuccessMessage = InformationMessages.Saved;
        }
        else
        {
            VehicleType.ModifiedBy = User.Identity.Name;
            await _vehicleTypeService.UpdateVehicleType(VehicleType.Id, VehicleType, logModel);
