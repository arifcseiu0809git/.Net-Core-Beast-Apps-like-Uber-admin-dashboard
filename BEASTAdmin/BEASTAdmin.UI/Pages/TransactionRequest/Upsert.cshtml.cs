...
public partial class UpsertModel : PageModel
{
    ...
    [BindProperty, DisplayName("TransactionRequest Image")]
    public IFormFile? TransactionRequestImage { get; set; }
	...
	public UpsertModel(ILogger<UpsertModel> logger, IConfiguration config, TransactionRequestService transactionRequestService, CategoryService categoryService, IWebHostEnvironment hostEnvironment)
	{
        this._logger = logger;
        this._config = config;
    ...
    TryCatch(async () =>
    {
        ...
        {
            TransactionRequest = new TransactionRequestModel();
        }
        else
        {
            TransactionRequest = await _transactionRequestService.GetTransactionRequestById(id);
        }
    ...
    TryCatch(async () =>
    {
        ...
        if (string.IsNullOrEmpty(TransactionRequest.Id))
        {
            TransactionRequest.CreatedBy = User.Identity.Name;
            await _transactionRequestService.InsertTransactionRequest(TransactionRequest, logModel);
            SuccessMessage = InformationMessages.Saved;
        }
        else
        {
            TransactionRequest.ModifiedBy = User.Identity.Name;
            await _transactionRequestService.UpdateTransactionRequest(TransactionRequest.Id, TransactionRequest, logModel);
