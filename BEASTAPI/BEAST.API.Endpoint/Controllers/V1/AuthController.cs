...
public partial class AuthController : ControllerBase
{
    ...
    public AuthController(ILogger<AuthController> logger, IConfiguration config, ISecurityHelper securityHelper, IAuthRepository authRepository, UserManager<ApplicationUser> userManager)
    {
        this._logger = logger;
        this._config = config;
    ...
    TryCatch(async () =>
    {
        ...
        {
            JwtToken = await Task.Run(() => _securityHelper.GenerateJSONWebToken(userData)),
        ...
        await _authRepository.UpdateRefreshToken(userData.Id, token);
        return Ok(token);
    });
