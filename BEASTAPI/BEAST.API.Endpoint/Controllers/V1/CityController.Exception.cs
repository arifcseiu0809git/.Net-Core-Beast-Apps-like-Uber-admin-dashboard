...
public partial class CityController
{
    ...
    private async Task<IActionResult> TryCatch(ReturningFunction returningFunction)
    {
        ...
        catch (Exception ex)
        {
            ...
            if (returningFunction.Method.Name.Contains("GetCities"))
                Messages = ExceptionMessages.Pie_List;

            if (returningFunction.Method.Name.Contains("GetCityById"))
                Messages = ValidationMessages.Pie_NotFoundId;

            if (returningFunction.Method.Name.Contains("InsertCity"))
                Messages = ExceptionMessages.Pie_Insert;

            if (returningFunction.Method.Name.Contains("UpdateCity"))
                Messages = ExceptionMessages.Pie_Update;

            if (returningFunction.Method.Name.Contains("DeleteCity"))
