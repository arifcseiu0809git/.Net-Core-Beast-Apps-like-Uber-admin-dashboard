...
public partial class EmailTemplateController : ControllerBase
{
    ...
    [HttpGet("{id}"), Authorize(Policy = Constants.SystemAdmin)]
    public Task<IActionResult> GetEmailTemplateById(string id) =>
    TryCatch(async () =>
    {
        #region Validation
        if (Convert.ToBoolean(_config["Hash:HashChecking"]))
    ...
    [HttpPut("Update/{id}"), Authorize(Policy = Constants.SystemAdmin)]
    public Task<IActionResult> UpdateEmailTemplate(string id, [FromBody] Dictionary<string, object> PostData) =>
    TryCatch(async () =>
    {
