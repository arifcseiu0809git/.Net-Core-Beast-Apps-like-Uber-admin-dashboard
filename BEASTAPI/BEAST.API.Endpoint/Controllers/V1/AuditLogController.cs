...
public partial class AuditLogController : ControllerBase
{
    ...
    [HttpGet("{id}")]
    public Task<IActionResult> GetAuditLogById(string id) =>
    TryCatch(async () =>
    {
        #region Validation
        if (Convert.ToBoolean(_config["Hash:HashChecking"]))
    ...
    TryCatch(async () =>
    {
        ...
        string insertedAuditLogId = await _auditLogRepository.InsertAuditLog(logModel);
        return Created(nameof(GetAuditLogById), new { id = insertedAuditLogId });
    });

    [HttpDelete("{id}")]
    public Task<IActionResult> DeleteAuditLog(string id) =>
    TryCatch(async () =>
    {
        #region Validation
        if (Convert.ToBoolean(_config["Hash:HashChecking"]))
