...
public partial class RatingController : ControllerBase
{
    ...
    public RatingController(SecurityHelper securityHelper, ILogger<PieController> logger, IConfiguration config, ISMSRepository smsRepository, ICsvExporter csvExporter)
    {
    ...
    [HttpGet]
    public Task<IActionResult> GetRatings(int pageNumber) =>
    TryCatch(async () =>
    {
    ...
    [HttpGet("{otpId}")]
    public Task<IActionResult> GetRatingById(string otpId) =>
    TryCatch(async () =>
    {
        if (Convert.ToBoolean(_config["Hash:HashChecking"]))
        {
    ...
    [HttpPost]
    public Task<IActionResult> InsertRating([FromBody] Dictionary<string, object> PostData) =>
    TryCatch(async () =>
    {
        ...
        string insertedRating = await _smsRepository.InsertSMS(sms, logModel);
        return Created(nameof(GetRatingById), new { id = insertedRating });
    });

    [HttpPut("Update/{id}")]
    public Task<IActionResult> UpdateRating(string id, [FromBody] Dictionary<string, object> PostData) =>
    TryCatch(async () =>
    {
        if (Convert.ToBoolean(_config["Hash:HashChecking"]))
        {
    ...
    [HttpPut("Delete/{id}")]
    public Task<IActionResult> DeleteRating(string id, [FromBody] LogModel logModel) =>
    TryCatch(async () =>
    {
        if (Convert.ToBoolean(_config["Hash:HashChecking"]))
        {
