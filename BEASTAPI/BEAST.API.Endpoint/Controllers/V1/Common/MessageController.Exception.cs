...
public partial class MessageController
{
    ...
    private async Task<IActionResult> TryCatch(ReturningFunction returningFunction)
    {
        ...
        catch (Exception ex)
        {
            ...
            if (returningFunction.Method.Name.Contains("GetMessages"))
                Messages = ExceptionMessages.Pie_List;

            if (returningFunction.Method.Name.Contains("GetMessageById"))
                Messages = ValidationMessages.Pie_NotFoundId;

            if (returningFunction.Method.Name.Contains("InsertMessage"))
                Messages = ExceptionMessages.Pie_Insert;

            if (returningFunction.Method.Name.Contains("UpdateMessage"))
                Messages = ExceptionMessages.Pie_Update;

            if (returningFunction.Method.Name.Contains("DeleteMessage"))
