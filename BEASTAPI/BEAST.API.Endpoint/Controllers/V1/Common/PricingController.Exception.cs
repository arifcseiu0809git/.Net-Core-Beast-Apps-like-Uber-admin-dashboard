...
public partial class PricingController
{
    ...
    private async Task<IActionResult> TryCatch(ReturningFunction returningFunction)
    {
        ...
        catch (Exception ex)
        {
            ...
            if (returningFunction.Method.Name.Contains("GetPricings"))
                Messages = ExceptionMessages.Pie_List;

            if (returningFunction.Method.Name.Contains("GetPricingById"))
                Messages = ValidationMessages.Pie_NotFoundId;

            if (returningFunction.Method.Name.Contains("InsertPricing"))
                Messages = ExceptionMessages.Pie_Insert;

            if (returningFunction.Method.Name.Contains("UpdatePricing"))
                Messages = ExceptionMessages.Pie_Update;

            if (returningFunction.Method.Name.Contains("DeletePricing"))
